...
void setup() {
  ...
  map.setCenterZoom(new Location(newLat, newLong), 11);  
...
void draw() {
  ...
  if (gui) {
    ...
    location = new Location(newLat, newLong);
...
//      accelX = map(theOscMessage.get(0).floatValue(), -3.0, 3.0, -200.0, 200.0);
//      accelY = map(theOscMessage.get(1).floatValue(), -3.0, 3.0, -200.0, 200.0);
//      accelZ = map(theOscMessage.get(2).floatValue(), -3.0, 3.0, -200.0, 200.0);
//    }
//  }
//  
    // Check to see if the address of the OscMessage is accellerometer data. Press "?" on the GyrOSC app to see the addresses
  ...
  {
      newLat = map(theOscMessage.get(0).floatValue(), -3.0, 3.0, -200.0, 200.0);
      newLong = map(theOscMessage.get(1).floatValue(), -3.0, 3.0, -200.0, 200.0);
  }
